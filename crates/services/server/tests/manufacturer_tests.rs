...
use crate::common::seeding::seed_manufacturers;
...
const API_BRANDS: &str = "/api/manufacturers";

#[tokio::test]
async fn it_should_return_409_when_the_manufacturer_already_exists() {
    ...
    let manufacturer_name = "ACME";

    let reg = setup_hbs();
    let data = json!({"manufacturer_name": manufacturer_name});
    let request = render(reg, "manufacturers", data);
...
#[tokio::test]
async fn it_should_create_new_manufacturers() {
    let (_container, port) = start_postgres().await;
    let sut = spawn_app(port).await;
    let client = reqwest::Client::new();
    sut.run_database_migrations().await;

    let manufacturer_name = Uuid::new_v4().to_string();
    let expected_location = format!("{}/{}", API_BRANDS, manufacturer_name);
    ...
    let saved = sqlx::query_as!(Saved,
            r#"SELECT
                    manufacturer_id, name, registered_company_name, 
                    organization_entity_type as "organization_entity_type: OrganizationEntityType", group_name, 
                    description_de, description_en, description_fr, description_it, kind as "kind: ManufacturerKind", status as "status: ManufacturerStatus",
                ...
                FROM manufacturers WHERE name = $1"#, &manufacturer_name)
        .fetch_one(&pg_pool)
        .await
        .expect("Failed to fetch saved manufacturer.");

    assert_eq!(manufacturer_name, saved.manufacturer_id);
    assert_eq!(manufacturer_name, saved.name);
    assert_eq!(ManufacturerKind::Industrial, saved.kind);
    ...
    assert_eq!(Some(ManufacturerStatus::Active), saved.status);
...
#[tokio::test]
async fn it_should_find_manufacturers_by_id() {
    let (_container, port) = start_postgres().await;
    let sut = spawn_app(port).await;
    let client = reqwest::Client::new();
    sut.run_database_migrations().await;
    ...
    assert_eq!(ManufacturerId::new("ACME"), body.manufacturer_id);
    assert_eq!("ACME", body.name);
    assert_eq!(ManufacturerKind::Industrial, body.kind);
    ...
    assert_eq!(Some(ManufacturerStatus::Active), body.status);
...
#[tokio::test]
async fn it_should_return_404_not_found_when_the_manufacturer_is_not_found() {
    let (_container, port) = start_postgres().await;
    let sut = spawn_app(port).await;
    let client = reqwest::Client::new();
    sut.run_database_migrations().await;
...
#[tokio::test]
async fn it_should_return_the_manufacturers_list() {
    let (_container, port) = start_postgres().await;
    let sut = spawn_app(port).await;
    let client = reqwest::Client::new();
    sut.run_database_migrations().await;
...
#[tokio::test]
async fn it_should_return_200_with_empty_result_set_when_no_manufacturer_is_found() {
    let (_container, port) = start_postgres().await;
    let sut = spawn_app(port).await;
    let client = reqwest::Client::new();
    sut.run_database_migrations().await;
